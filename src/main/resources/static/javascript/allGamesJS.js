    ...
    if (userId != null) {
        apiService.getAll("/membrosGamesAvaliacoes", function(status, response){
            ...
            for(var j=0; j<response.length; j++) {
                ...
                border-radius: 20px; padding-bottom: 50px; text-align:center;">
                    <div id='cabecalho'>
                        <p id='data'>${final_date}</p>
                        <p id='userName'>${avaliacao.membro.username}</p>
                    </div>
                    <textarea cols='60' rows='8' id='textoAva' style="background-color:lightgray; color:white; border: none;
                    outline: none; text-align:center;">${avaliacao.avaliacao.texto}</textarea>
